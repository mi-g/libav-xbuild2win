...
(cd $SRCDIR; git clone git://diracvideo.org/git/schroedinger.git schro; cd schro; git checkout "$SCHRO_VER")
...
(cd $SRCDIR; git clone git://git.openssl.org/openssl.git ssl; cd ssl; git checkout "$SSL_VER")
